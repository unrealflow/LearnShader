...
vec4 DrawEmit0(vec2 coord)
{
    ...
    vec4 data=vec4(0.0,0.5,0.0,0.0);
    float m2=dot(target,target);
    return data*smoothstep(0.001,0.0,m2);
}
...
void mainImage(out vec4 fragColor,in vec2 fragCoord)
{
    ...
    data+=DrawEmit0(coord);
    data+=DrawEmit1(coord);

    data.x *= smoothstep(.5,.48,abs(uv.x-0.5)); //Boundaries
    data = clamp(data, vec4(vec2(-5), 0.5 , -10.), vec4(vec2(5), 3.0 , 10.));
